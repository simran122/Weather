...
function App(){
  ...
  useEffect(()=> {
    dispatch(fetchWeatherAction('new delhi'))
  }, []);
      ...
      <section class ="relative bg-gray-900 min-h-screen">
        ...
        <div class ="relative container pt-12 px-4 mb-20 mx-auto text-center">
          ...
          <input 
            ...
            class="relative z-10 inline-block w-full md:w-auto mb-2 px-3 py-2 mr-4 font-medium leading-normal bg-transparent border-2 rounded-lg text-green-400"></input>
          <button
            onClick={()=>dispatch(fetchWeatherAction(city))}
            type="button"
            className="inline-flex items-center px-3 pr-3 28 text-center py-3 
            ...
            <div class="w-full md:w-1/3 px-4">
              <div class="p-8 border border-blue-800 rounded-lg">
                ...
                <p class="mb-8 text-gray-300">
                   This weather condition in {weather?.name},
                  {weather?.sys?.country} is described as : {""}
                  {weather?.weather[0].description} with a temperature of {""}
                  {Math.round(Number(weather?.main?.temp-273))} °C and a humidity of {""} {weather?.main?.humidity} % 
                </p>
                <a 
                  class="ml-auto flex-items-center justify-center w-20 h-20 rounded-full hover:bg-blue-700 text-white" href="#">
                  <span class="flex-items-center justify-center w-26 h-16 rounded-full border-2">
                    {/* weather logo*/}
                    <img
                        class="w-56"
